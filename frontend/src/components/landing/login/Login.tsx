...
const Login = () => {
  ...
  const onSubmit: SubmitHandler<LoginInputs> = async ({ email, password }: LoginInputs) => {
    ...
    try {
      const response = await http.post('/token/obtain/', { email, password, username: email })
            ...
            <form className="space-y-6" onSubmit={handleSubmit(onSubmit)}>
              <div>
                ...
                <div className="mt-1">
                  ...
                  />
                  {serverErrors.detail && <span className="input-error">{serverErrors.detail}</span>}
              ...
              <div className="text-sm">
                <a href="http://#" className="font-medium text-green1 hover:text-green2" onClick={() => history.push("/reset")}>
